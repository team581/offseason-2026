...
public class Robot extends Base581Robot {
	...
	private final Trailblazer trailblazer = new Trailblazer(new HeuristicPathTracker(new PoseErrorTolerance(0.5, 10)),
			new BumpCrossingFollower(new PidPathFollower(new PIDController(3.5, 0, 0),
					new PIDController(Swerve.ORIGINAL_HEADING_PID.getP(), Swerve.ORIGINAL_HEADING_PID.getI(),
							Swerve.ORIGINAL_HEADING_PID.getD())),
					imu.bumpCrossingTracker));

	private final HealthManager health = new HealthManager(hardware.shooterLimeLight, hardware.leftLimeLight,
			hardware.rightLimeLight, hardware.groundLimeLight);
	private final Vision vision = new Vision(imu, hardware.shooterLimeLight, hardware.leftLimeLight,
			hardware.rightLimeLight, hardware.groundLimeLight);
	private final Swerve swerve = new Swerve(hardware.drivetrain, health, hardware.driverController, trailblazer);
	private final Localization localization = new Localization(swerve, hardware.drivetrain, vision, imu);

	private final ShooterHood shooterHood = new ShooterHood(hardware.shooterHoodMotor);
	private final Shooter shooter = new Shooter(hardware.shooterTopLeftMotor, hardware.shooterTopRightMotor,
			hardware.shooterBottomLeftMotor, hardware.shooterBottomRightMotor);
	...
	private final PowerManager powerManager = new PowerManager(shooter, null, null, shooterHood, null, null, swerve);
	private final RobotManager robotManager = new RobotManager(null, shooterHood, localization, swerve, shooter, vision,
			hardware.driverController, health, hubActivity, trailblazer, null, hardware, powerManager, turret);

	public Robot() {
		logMetadata(BuildConstants.MAVEN_NAME, BuildConstants.BUILD_DATE, BuildConstants.GIT_SHA,
				BuildConstants.GIT_DATE, BuildConstants.GIT_BRANCH, BuildConstants.DIRTY);
