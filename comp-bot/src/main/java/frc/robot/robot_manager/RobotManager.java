...
public class RobotManager extends StateMachineSubsystem<RobotState> {
	...
	public RobotManager(ShooterHood shooterHood, Localization localization, Swerve swerve, Shooter shooter,
			DyeRotor dyeRotor, Turret turret, GenericIntake intake, Deploy deploy, Vision vision,
			XboxController driverController, HealthManager health, HubActivity hubActivity, Trailblazer trailblazer,
			Climber climber, ClusterMap clusterMap, Hardware hardware) {
  ...
  protected RobotState getNextState(RobotState currentState) {
	...
	protected void afterTransition(RobotState newState) {
		switch (newState) {
			...
			case PREPARE_FORCE_SCORE -> {
				...
				dyeRotor.idleRequest();
				turret.scoreRequest(scoringParameters.turretAngle(), scoringParameters.turretFeedForwardRadians());
				swerve.normalDriveRequest();
				climber.stowRequest();
			}
			case FORCE_SCORE -> {
			...
			case FEED -> {
				...
				turret.feedRequest(feedingParameters.turretAngle(), feedingParameters.turretFeedForwardRadians());
				if (intake.getState().isIntaking()) {
					intake.shootThenIntakeRequest();
				} else {
					intake.shootRequest();
			...
			case AUTOMATIC_CLIMB_1_APPROACH_L1 -> {
				...
				intake.idleRequest();
				trailblazer
						.setActiveSegment(ClimbAssist.getApproachClimbAssistSegment(robotPose, ClimbLocation.CLOSEST));
			...
			case AUTOMATIC_CLIMB_2_LINEUP_L1 -> {
				...
				deploy.stowRequest();
				intake.idleRequest();
				trailblazer.setActiveSegment(ClimbAssist.getLineupClimbAssistSegment(robotPose, ClimbLocation.CLOSEST));
				climber.l1LineupRequest();
			}
			case AUTOMATIC_CLIMB_3_HANGING_L1 -> {
				vision.setState(VisionState.TAGS);
	...
	protected void whileInState(RobotState state) {
		switch (state) {
			...
			case AUTOMATIC_CLIMB_3_HANGING_L1, AUTOMATIC_CLIMB_4_RAISING_L2, AUTOMATIC_CLIMB_5_HANGING_L2,
					AUTOMATIC_CLIMB_6_RAISING_L3, AUTOMATIC_CLIMB_7_HANGING_L3, CLIMB_1_LINEUP_L1_AUTONOMOUS,
					CLIMB_2_RAISING_L1_AUTONOMOUS, CLIMB_3_HANGING_L1_AUTONOMOUS, CLIMB_4_RELEASE_L1_AUTONOMOUS,
					MANUAL_CLIMB_1_LINEUP_L1, MANUAL_CLIMB_2_HANGING_L1, MANUAL_CLIMB_3_RAISING_L2,
					MANUAL_CLIMB_4_HANGING_L2, MANUAL_CLIMB_5_RAISING_L3, MANUAL_CLIMB_6_HANGING_L3 -> {
				// TODO: Use actual feed forward
				turret.climbRequest(robotPose, 0);
			}
			default -> {
		...
		MechanismVisualizer.log(robotPose, turret.getAngle(), shooterHood.getAngle(), deploy.getPosition(),
				climber.getHeight(), dyeRotor.getAngle());
	}
	...
	private void smartTurretHoodIdleRequest() {
		...
		if (!health.isLocalizationHealthy() || !localization.isTrustworthy() || nearTrench) {
			shooterHood.idleRequest();
			turret.idleScoreRequest(scoringParameters.turretAngle(), scoringParameters.turretFeedForwardRadians());
			...
			shooterHood.idleRequest();
			turret.idleFeedRequest(feedingParameters.turretAngle(), feedingParameters.turretFeedForwardRadians());
	...
	public void cancelIntakeRequest() {
		...
		switch (getState()) {
			case PREPARE_FORCE_SCORE, FORCE_SCORE, PREPARE_PRESET_SCORE, PRESET_SCORE, PREPARE_SCORE, SCORE,
					PREPARE_PRESET_FEED, PRESET_FEED, PREPARE_FEED, FEED ->
				deploy.shuffleRequest();
			default -> {
			}
		}
	...
	public void stopTeleopAutoClimbAlignment() {
		switch (getState()) {
			case AUTOMATIC_CLIMB_1_APPROACH_L1, AUTOMATIC_CLIMB_2_LINEUP_L1 -> setStateFromRequest(RobotState.IDLE);
			default -> {
			}
		}
	}
	...
	public void manualClimbSequenceBackwardOrIdleRequest() {
		switch (getState()) {
			...
			case CLIMB_2_RAISING_L1_AUTONOMOUS -> setStateFromRequest(RobotState.CLIMB_1_LINEUP_L1_AUTONOMOUS);
			case CLIMB_3_HANGING_L1_AUTONOMOUS -> setStateFromRequest(RobotState.CLIMB_2_RAISING_L1_AUTONOMOUS);

			// This is the last step in the climb sequence, so just go to stowed
			case MANUAL_CLIMB_1_LINEUP_L1, AUTOMATIC_CLIMB_1_APPROACH_L1 -> setStateFromRequest(RobotState.IDLE);
	...
	protected void collectInputs() {
		...
		nearTrench = (Point.CLAMPED_POINTS_FEATURE_FLAG.getAsBoolean()
		...
		scoringParameters = AimParameterUtil.getScoringParameters(
				health.isLocalizationHealthy()
						? robotPose
						: new Pose2d(FieldUtil.getFallbackScorePoint().getTranslation(), robotPose.getRotation()),
				swerve.getFieldRelativeSpeeds());

		if (!DSOptions.USE_TURRET.getAsBoolean()) {
			scoringParameters = AimParameterUtil
					.getTurretStuckScoringParameters(
							health.isLocalizationHealthy()
									? robotPose
									: new Pose2d(FieldUtil.getFallbackScorePoint().getTranslation(),
											robotPose.getRotation()),
							turret.getAngle(), swerve.getFieldRelativeSpeeds());
		}

		shooter.getScoreTimeOfFlight(scoringParameters.distance());
		if (!health.isLocalizationHealthy()) {
			feedingParameters = AimParameterUtil.getFallbackFeedingParameters(feedLocation, robotPose,
					swerve.getFieldRelativeSpeeds());
		} else {
			feedingParameters = AimParameterUtil.getFeedingParameters(feedLocation, robotPose,
					swerve.getFieldRelativeSpeeds());
		}

		var swerveVector = MathHelpers.getDriveDirection(speeds);
		double driveDirection = swerveVector.getDegrees();
		drivingToIntake = intake.getState().isIntaking()
				&& MathUtil.isNear(robotRotation, driveDirection, 120.0, -180, 180)
				&& MathHelpers.getLinearVelocity(speeds) > 1e-5;
		isInScoringZone = !health.isLocalizationHealthy()
				|| !FieldUtil.isInNoScoreZone(TurretCalculator.getTurretPose(robotPose));

		isInAllianceZone = FieldUtil
				.isRobotPastObstacleTowardAllianceZone(TurretCalculator.getTurretPose(robotPose).getTranslation());
	}

	private void logScoringTransition() {
		DogLog.log("RobotManager/Scoring/ScoreTransition/ShooterAtGoal", shooter.atGoal());
		DogLog.log("RobotManager/Scoring/ScoreTransition/LocalizationTrustworthy", localization.isTrustworthy());
		DogLog.log("RobotManager/Scoring/ScoreTransition/InAllianceZone",
				FieldUtil.isRobotInAllianceZone(robotPose.getTranslation()));
		DogLog.log("RobotManager/Scoring/ScoreTransition/DyeRotorNotJammed", !dyeRotor.isJammed());
		DogLog.log("RobotManager/Scoring/ScoreTransition/TurretAtGoal", turret.atGoal(scoringParameters));
	...
	private void logFeedTransition() {
		...
		DogLog.log("RobotManager/Feeding/FeedTransition/LocalizationHealthy", health.isLocalizationHealthy());
		DogLog.log("RobotManager/Feeding/FeedTransition/InNoFeedZone",
				!FieldUtil.isRobotInNoFeedZone(TurretCalculator.getTurretPose(robotPose)));
		DogLog.log("RobotManager/Feeding/FeedTransition/DyeRotorNotJammed", !dyeRotor.isJammed());
		DogLog.log("RobotManager/Feeding/FeedTransition/TurretAtGoal", turret.atGoal(feedingParameters));
