...
def parse_args():
    parser = argparse.ArgumentParser(description="Analyze current draw from CTRE .hoot files")
    parser.add_argument("hoot_files", nargs='+', help="Path to one or more .hoot files to parse")
    parser.add_argument("--high-current-threshold", type=float, default=150.0,
                        help="Threshold for total current to be considered 'high' (Amps)")
    parser.add_argument("--force-convert", action="store_true", help="Force reconversion of the .hoot file even if a .wpilog exists")
...
def load_log(filepaths):
    ...
    for path in filepaths:
        ...
        for record in reader:
            if record.isStart():
                ...
                is_time = entry_name in ["/Robot/HubActivity/TimeSinceMatchStart", "/Robot/HubActivity/TimeSinceTeleopEnable"]

                if is_motor or is_time:
                    time_s = record.getTimestamp() / 1_000_000.0  # microseconds to seconds
                    value = None
                    ...
                    if value is not None:
                        all_data.append({"timestamp": time_s, "name": entry_name, "value": value})
    ...
    df_pivot = df.pivot_table(index="timestamp", columns="name", values="value", aggfunc="mean")
...
def analyze_incidents(mech_currents, threshold):
    ...
    for s, e in zip(starts, ends):
        ...
        incidents.append({"start": s, "end": e, "duration": duration, "max_current": max_current})
...
def get_match_time_string(df, timestamp_s):
    ...
    if "/Robot/HubActivity/TimeSinceTeleopEnable" in df.columns and "/Robot/HubActivity/TimeSinceMatchStart" in df.columns:
        ...
        tele_time = df.loc[closest_stamp, "/Robot/HubActivity/TimeSinceTeleopEnable"] if "/Robot/HubActivity/TimeSinceTeleopEnable" in df.columns else -1
        match_time = df.loc[closest_stamp, "/Robot/HubActivity/TimeSinceMatchStart"] if "/Robot/HubActivity/TimeSinceMatchStart" in df.columns else -1
...
def generate_report_and_plots(df_raw, mech_currents, incidents):
    ...
    avg_currents = mech_currents.drop(columns=["Total_Supply_Current"]).mean().sort_values(ascending=False)
    ...
    if not incidents:
        ...
        for idx, inc in enumerate(incidents[:5]):
            ...
            print(f"  - Incident {idx+1}: Duration {inc['duration']:.2f}s (Start: {inc['start']:.2f}s [{ctx}], Peak: {inc['max_current']:.2f}A)")
    ...
    peak_breakdown = mech_currents.drop(columns=["Total_Supply_Current"]).loc[peak_stamp].sort_values(ascending=False)
    ...
    plt.subplot(2, 1, 1)
    plt.plot(mech_currents.index, mech_currents["Total_Supply_Current"], label="Total Current", color="black", linewidth=1.5)
    ...
    plt.stackplot(mech_only.index, [mech_only[col] for col in mech_only.columns], labels=mech_only.columns, alpha=0.8)
