...
public class ShooterHood extends StateMachineSubsystem<ShooterHoodState> implements PowerManaged {
  ...
  private final VoltageOut homingVoltageRequest = new VoltageOut(0).withEnableFOC(false);
  ...
  protected void afterTransition(ShooterHoodState newState) {
    switch (newState) {
      case HOMING ->
          motor.setControl(homingVoltageRequest.withOutput(ShooterHoodConfig.HOMING_VOLTAGE));
